...
def format_date_dmy(value):
    ...
    # НОВА ПРОВЕРКА: Ако стойността е от тип bytes, опитай да я декодираш.
    if isinstance(value, bytes):
        try:
            value = value.decode('utf-8')
        except UnicodeDecodeError:
            # Ако декодирането не успее, връщаме суровата репрезентация, за да не се срине.
            print(f"ПРЕДУПРЕЖДЕНИЕ: Не може да се декодира byte string: {value!r}")
            return repr(value)

    date_obj = None
    try:
        # Първо проверяваме дали вече не е date/datetime обект
        if isinstance(value, datetime):
            date_obj = value
        elif isinstance(value, date):
            date_obj = datetime.combine(value, datetime.min.time())
        else:
            # Преобразуваме към стринг и опитваме да парснем ISO формат
            str_value = str(value)
            # Премахваме микросекундите, ако има такива, тъй като fromisoformat се справя по-добре без тях понякога
            if '.' in str_value:
                str_value = str_value.split('.')[0]
            
            # Пробваме няколко често срещани формата
            ...
            # Ако никой формат не е сработил, вдигаме грешка, за да се улови от външния try-except
            if not date_obj:
                 raise ValueError("Нито един от форматите не съвпадна")

    except (ValueError, TypeError):
        # Ако нищо не успее, отпечатва предупреждение и връща оригиналната стойност
        print(f"ПРЕДУПРЕЖДЕНИЕ: Не може да се форматира стойност за дата: '{value}'")
        return value

    # Проверява дали има информация за час, минута, секунда
...
def calculate_fine(due_date_str):
    ...
    try:
        ...
        if date.today() > due_date:
            ...
            return round(overdue_days * current_app.config['FINE_PER_DAY'], 2)
    except (ValueError, TypeError):
