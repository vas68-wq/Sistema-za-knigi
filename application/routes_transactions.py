...
from .utils import login_required, log_activity, calculate_fine, format_date_dmy
...
def process_borrow():
    ...
    due_date = borrow_date.date() + timedelta(days=20)
    ...
    if book and reader:
        ...
        flash(f"Книга '{book['title']}' е заета успешно от '{reader['full_name']}'.", 'success')
...
def api_borrowed_books():
    ...
    # КОРЕКЦИЯ: Взимаме всички заети книги от базата данни
    base_sql = """
    ...
    # КОРЕКЦИЯ: Филтрираме резултатите в Python, което е 100% надеждно
    if search_query:
    ...
    # Добавяме изчислените полета към филтрирания списък
