...
from .utils import login_required, log_activity, clean_int, clean_price, allowed_file, clean_date
...
@login_required
def books_list_page():
    """Рендерира само празната страница, данните и жанровете се зареждат динамично."""
    return render_template('books_list.html')

@books_bp.route('/api/books')
@login_required
def api_books():
    """API ендпойнт, който връща книгите като JSON, с търсене и пагинация."""
    ...
    # Вземаме и жанровете, за да ги подадем към модалния прозорец
...
def book_details_page(tom_no):
    ...
    borrow_history = []
    if 'user_id' in session:
        borrow_history = conn.execute("SELECT br.*, r.full_name, r.reader_no FROM borrows br JOIN readers r ON br.reader_no = r.reader_no WHERE br.book_tom_no = ? ORDER BY br.borrow_date DESC", (tom_no,)).fetchall()

    return render_template('book_details.html', book=book, borrow_history=borrow_history)
...
@books_bp.route('/edit_book/<path:tom_no>', methods=['GET', 'POST'])
@login_required
def edit_book_page(tom_no):
    conn = get_db()
    if request.method == 'POST':
        back_url = request.form.get('back_url')

        final_genre = request.form.get('new_genre') or request.form.get('genre_select')
        if request.form.get('new_genre'):
             conn.execute("INSERT OR IGNORE INTO genres (name) VALUES (?)", (final_genre,))
        ...
        return redirect(url_for('books.book_details_page', tom_no=tom_no))
